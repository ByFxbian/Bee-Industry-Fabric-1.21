...
public class DirtNestBlock extends BlockWithEntity {
    public static final MapCodec<DirtNestBlock> CODEC = createCodec(DirtNestBlock::new);


    public DirtNestBlock(Settings settings) {
        super(settings);
    }
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new DirtNestBlockEntity(pos, state);
    }
        ...
        if(!world.isClient){
            if(player.getStackInHand(Hand.MAIN_HAND).isOf(BeeIndustryItems.BEE_SMOKER)) {
                ...
                if(beeToSpawn != null) {
                    ...
                    registry.getEntry(CustomBees.DIRT).ifPresent(beeToSpawn::setCustomBee);
