...
import at.byfxbian.beeindustry.block.entity.custom.SandNestBlockEntity;
...
public class SandNestBlock extends BlockWithEntity {
    public static final MapCodec<SandNestBlock> CODEC = createCodec(SandNestBlock::new);

    public SandNestBlock(Settings settings) {
        super(settings);
    }
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new SandNestBlockEntity(pos, state);
    }
        ...
        if(!world.isClient){
            if(player.getStackInHand(Hand.MAIN_HAND).isOf(BeeIndustryItems.BEE_SMOKER)) {
                ...
                if(beeToSpawn != null) {
                    ...
                    registry.getEntry(CustomBees.SAND).ifPresent(beeToSpawn::setCustomBee);
