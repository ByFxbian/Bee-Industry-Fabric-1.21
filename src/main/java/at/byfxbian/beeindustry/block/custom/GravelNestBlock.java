...
import at.byfxbian.beeindustry.block.entity.custom.GravelNestBlockEntity;
...
public class GravelNestBlock extends BlockWithEntity {
    public static final MapCodec<GravelNestBlock> CODEC = createCodec(GravelNestBlock::new);

    public GravelNestBlock(Settings settings) {
        super(settings);
    }
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new GravelNestBlockEntity(pos, state);
    }
        ...
        if(!world.isClient){
            if(player.getStackInHand(Hand.MAIN_HAND).isOf(BeeIndustryItems.BEE_SMOKER)) {
                ...
                if(beeToSpawn != null) {
                    ...
                    registry.getEntry(CustomBees.GRAVEL).ifPresent(beeToSpawn::setCustomBee);
