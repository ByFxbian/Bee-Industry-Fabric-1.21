...
public class BeekeeperArmorItem extends ArmorItem {
    private static final Map<RegistryEntry<ArmorMaterial>, List<StatusEffectInstance>> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<RegistryEntry<ArmorMaterial>, List<StatusEffectInstance>>())
                    .put(BeeIndustryArmorMaterials.BEEKEEPER_ARMOR_MATERIAL,
                            List.of(new StatusEffectInstance(StatusEffects.HASTE, 400, 1, false, false))).build();

    public BeekeeperArmorItem(RegistryEntry<ArmorMaterial> material, Type type, Settings settings) {
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        ...
        if(!world.isClient() && entity instanceof PlayerEntity player) {
            ...
            ItemStack equippedStack = player.getEquippedStack(this.type.getEquipmentSlot());
            ...
            if (equippedStack.isOf(this)) {
                switch (this.type) {
                    ...
                    case LEGGINGS:
                        break;
                    case BOOTS:
                        if (world.getBlockState(player.getBlockPos().down()).isIn(BlockTags.DIRT)) {
